...
class authService {
  ...
  async signup({ userName, email, password, age, phone }: signupDto): Promise<IUser> {
    ...
    const result = await this.userRepository.create({
      data: { userName, email, password: await hashPassword({ plainText: password }), age, phone: await encrypt(phone) }
    })
  ...
  async confirmEmail({ email, otp }: { email: string, otp: string }): Promise<void> {
    ...
    const hashOtp = await this.redis.get(this.redis.otpKey({ email, subject: EmailTypeEnum.confirmEmail }))
    if (!hashOtp) {
